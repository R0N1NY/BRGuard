...
def travelspeed_avg(game_travel_data):
    average_speed_data = []
    player_data = {}
    
    for data in game_travel_data:
        player_id = data['PlayerID']
        match_id = data['MatchID']
        speed = data['TravelSpeed']

        if player_id not in player_data:
            player_data[player_id] = {'total_speed': 0, 'count': 0}

        player_data[player_id]['total_speed'] += speed
        player_data[player_id]['count'] += 1

    for player_id, stats in player_data.items():
        if stats['count'] > 0:
            if stats['count'] > 1:
                average_speed = stats['total_speed'] / (stats['count'] - 1)
            else:
                average_speed = stats['total_speed'] / stats['count']
            average_speed_data.append({
                'PlayerID': player_id,
                'MatchID': match_id,
                'TravelSpeedAVG': average_speed
            })
        ...
        if os.path.isdir(game_folder_path):
            ...
            for player_csv in os.listdir(game_folder_path):
                if csv_file_pattern.search(player_csv):
                    ...
                    print(f'Processing TravelSpeed: {player_csv}')
            ...
            if game_travel_data:
                ...
                # # average travel speed data
                # avg_speed_data = travelspeed_avg(game_travel_data)
                # output_avg_file_path = os.path.join(game_folder_path, f'{game_folder}_TravelSpeedAVG_Structure.csv')
                # with open(output_avg_file_path, 'w', newline='', encoding='utf_8_sig') as output_avg_file:
                #     fieldnames = ['PlayerID', 'MatchID', 'TravelSpeedAVG']
                #     writer = csv.DictWriter(output_avg_file, fieldnames=fieldnames)
                #     writer.writeheader()
                #     writer.writerows(avg_speed_data)
