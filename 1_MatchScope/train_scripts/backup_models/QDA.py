...
data = pd.read_csv(f'../dataset/final_train_{n}.csv')
data = data.dropna()
train_ids = data['ID']
X_train = data.drop(['isCheater', 'ID'], axis=1)
y_train = data['isCheater']

test_data = pd.read_csv(f'../dataset/final_val.csv')
...
model_save_path = os.path.join(script_dir, f'models/QDA_best_model_{n}.pkl')
...
train_result_path = os.path.join(script_dir, f'train/QDA_{n}.txt')
...
result_save_path = os.path.join(script_dir, f'results/QDA_confusion_matrix_{n}.pdf')
...
cm_display.plot()
plt.title('QDA_confusion_matrix')
...
pred_save_path = os.path.join(script_dir, f'val/QDA_pred_{n}.csv')
...
# Write the DataFrame to CSV
train_save_path = os.path.join(script_dir, f'train/QDA_train_{n}.csv')
