...
class BattleDataset(Dataset):
    def __init__(self, dataframe, scaler=None):
        ...
        if scaler is None:
            ...
            self.scaler = scaler
        self.ids, self.sequences = self.create_sequences(dataframe, scaler is None)
    ...
    def __getitem__(self, idx):
        ...
        return id_, torch.tensor(sequence, dtype=torch.float), torch.tensor(label, dtype=torch.long)
...
def evaluate(model, dataloader, criterion, device, threshold=0.5):
    ...
    with torch.no_grad():
        for id_batch, X, y in dataloader:
    ...
    return ids, y_true, y_pred, y_probs, val_loss
...
def main():
    ...
    ids_test, y_true_test, y_pred_test, y_probs_test, test_loss = evaluate(model, test_loader, criterion, device, threshold=0.5)
    ...
    ids_val, y_true_val, y_pred_val, y_probs_val, val_loss = evaluate(model, valid_loader, criterion, device, threshold=0.5)
