...
model_save_path = os.path.join(script_dir, f'models/NaiveBayes_best_model_{n}.pkl')
...
result_txt_path = os.path.join(script_dir, 'test', f'NaiveBayes_{n}.txt')
create_directory_if_not_exists(os.path.dirname(result_txt_path))
with open(result_txt_path, 'w') as f:
    f.write(f"Best Model Parameters: {model.get_params()}\n")
...
conf_matrix_path = os.path.join(script_dir, 'results', 'results_test', f'NaiveBayes_{n}.pdf')
...
cm_display.plot()
plt.title('NaiveBayes_confusion_matrix')
plt.savefig(conf_matrix_path)
plt.show()
# plt.savefig('NB.png', dpi=300, format='png')
...
pred_csv_path = os.path.join(script_dir, 'test', f'NaiveBayes_test_pred_{n}.csv')
